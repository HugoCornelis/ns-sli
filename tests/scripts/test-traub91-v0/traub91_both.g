...
float tmax = 3.0               // simulation time in sec
...
function make_Vmgraph
    float vmin = -0.100
    float vmax = 0.05
    create xform /data [265,50,550,350]
...
function make_xcell
    create xform /cellform [820,50,400,400]
    ...
    setfield /cellform/draw/cell colmin -0.1 colmax 0.1 \
        path /asym_cell/##[TYPE=compartment] field Vm \
        script "echo <w> <v>"
end
...
function set_inject(dialog)
    ...
    setfield /asym_cell/soma inject {getfield {dialog} value}
end
...
readcell CA3.p /cell
readcell CA3_asym.p /asym_cell

setfield /cell/soma inject {injcurr}
setfield /asym_cell/soma inject {injcurr}
...
make_Vmgraph
addmsg /cell/soma /data/voltage PLOT Vm *sym *red
addmsg /asym_cell/soma /data/voltage PLOT Vm *asym *blue
...
if (hflag)
    ...
    create hsolve /asym_cell/solve
    setfield /asym_cell/solve path "/asym_cell/##[][TYPE=compartment]"
    setmethod 11
    setfield /cell/solve chanmode {chanmode}
    call /cell/solve SETUP
    setfield /asym_cell/solve chanmode {chanmode}
    call /asym_cell/solve SETUP
    reset
    echo "Using hsolve"
end
