...
/*======================================================================
  ...
  RSnet2-STDP.g is a version with spike timing dependent plasticity.

  ======================================================================*/

str script_name = "RSnet2-STDP.g"
str RUNID = "0000stdp"	    // default ID string for output file names
...
int use_stdp = 1 // Use spike timing dependent plasticity in Ex_ex connections
...
if(use_stdp)
    cellfile = "RScell_stdp.p"
    synpath = "soma/stdpAMPA" // stdpSynchan instead of synchan
end
...
function show_weights(maxrows)
    int row, maxrows
    str src, dst
    float weight
    floatformat %10.3f
    echo {getstat -time} -n // no new line
    // Note the horrible G2 SLI syntax to form src and dst strings
    for (row = 0; row < maxrows; row = row +1)
        src = "/network/cell[" @ {row*NX} @ "]"
        dst = "/network/cell[" @ {row*NX + 1} @ "]/" @ {synpath}
        weight = {getfield {dst} synapse[0].weight}
        echo {weight} -n
    end
    echo
    floatformat %0.10g
...
// set up a script_out to display selected weights
function make_weight_display
    int row, maxrows
    float show_weight_dt = tmax/20.0
    maxrows = 6
    create script_out /weight_inspector
    setfield /weight_inspector command "show_weights "{maxrows}
    setclock 3 {show_weight_dt}
    useclock /weight_inspector 3
end
...
function step_tmax
    ...
    echo "START: " {getdate}
    if(use_stdp)
        echo "Weight from cell at (0,y) to cell above on next row" \
            "for y = 0 to 5"
        echo "time" "weight[0]" "weight[1]" "weight[2]" "weight[3]" \
            "weight[4]" 	"weight[5]" -f %10s
    end
    pulse_hack
    echo "END  : " {getdate}
    do_run_summary
...
function make_prototypes
  ...
  */
  if(use_stdp)
	// Assume that stdp_functions4.g has been included
	extern make_stdpAMPA  // defined in stdp_functions4.g
	make_stdpAMPA /library/stdpAMPA
  end
...
if(use_stdp)
    include stdp_functions4.g
    minwt_stdp = 0.5  // must be <= 1
    maxwt_stdp = 5.0*syn_weight
end
...
if(use_stdp)
    // set up a VOLTAGE message from soma to stdpSynchan for spike discr
    int i
    for (i = 0 ; i < {NX*NY} ; i = i + 1)
	addmsg /network/cell[{i}]/soma \
            /network/cell[{i}]/{synpath} \
            VOLTAGE Vm
    end
    setfield /network/cell[]/{synpath}   Vspkmsg_idx 1
end
...
if(use_stdp)
    make_weight_display // set up a script_out to display selected weights
end
reset
