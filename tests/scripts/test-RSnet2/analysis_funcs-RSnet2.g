//genesis 2,3 - analysis_funcs.g and variants
...
/* Functions for network spike analysis
   ...
   This version, analysis_funcs-RSnet2.g, has been modified for RSnet2.g,
   a demonstration script for a grid of excitatory only cells connected
   to the four nearest neighbors.  Ninputs will be the number of rows,
   rows_per_octave = 0, and octave_distance will be irrelevant.
...
function make_freq_graph(xpos, ypos)
    ...
    float ymin = 0; float ymax = 250;
    create xform {form} [{xpos},{ypos},400,250]
    pushe {form}
    create xgraph frequency -hgeom 90% -bg white \
    ...
    create xdialog binwidth -wgeom 50% -ygeom 0:frequency -xgeom 0:rownum \
...
function make_freqmon
  ...
  int targ_row  // rows receiving thalamic input numbered 1 - Ninputs
  int n
  // This creates an extra /frequency_monitor[0] for average over all rows
  for (n = 0; n <= Ninputs; n = n + 1)
...
simulations, this will be the thalamic output for a sine wave tone burst at
...
function make_freqmon_messages(rownum)
    ...
    if (rownum > Ninputs)
        echo "Row number must be <= " {Ninputs}
        return
    end
...
/* Set up /frequency_monitor[1] through /frequency_monitor[Ninputs] */   
...
function make_freq_file
    ...
    int nlines = {round {tmax/freq_binwidth}} + 2 // plus t=0 and header
    int ncols = Ninputs + 1 // extra column for time
    call /spike_freq OUT_OPEN
    call /spike_freq OUT_WRITE {nlines}  {ncols}
    setfield /spike_freq append 1 // Prevent loss of header on reset
    useclock /spike_freq 3  // This is the binwidth

    for (targ_row = 1; targ_row <= Ninputs; targ_row = targ_row + 1)
...
function setup_rasterplot(n_per_row)  // assume all target rows
    ...
    int n_per_row
    // Target rows for MGBv inputs are numbered 1 through Ninputs
    int targ_min = 1; int targ_max = {Ninputs}
