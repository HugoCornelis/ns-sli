...
** This version sets up active compartments without synapses.
**********************************************************************/
...
/* separate function so we can have local variables */
    ...
    if (!({exists Purk_axon}))    // Lets copy before we add currents

        copy Purk_maind Purk_axon
    end
...
**  4455-4469, 1988. */
	...
	echo making Purkinje spines library...
    ...
    setfield Purk_spine Cm {{CM}*surf} Ra {4.0*{RA}*len/(dia*dia*{PI})}  \
        Em {ELEAK} initVm {EREST_ACT} Rm {{RMd}/surf} inject 0.0 x {len} \
         dia {dia} len {len}
    ...
    setfield Purk_spine/head Cm {{CM}*surf} Ra {8.0*{RA}/(dia*{PI})}  \
        Em {ELEAK} initVm {EREST_ACT} Rm {{RMd}/surf} inject 0.0 x {dia} \
         dia {dia} len {len}
