 ...
 *
 * Further modifications March 2010 by Dave Beeman to implement K_C_int
 * with a tabchannel instead of vdep_channel.
 */
...
function make_K_C_int
        ...
        create  tabchannel K_C_int
        setfield           ^                    \
                Ek              {E_K}    \
                Gbar            { 100.0 * INT_SOMA_A }      \
                Ik              0       \
                Gk              0
...
// Now make a X-table for the voltage-dependent activation parameter.
        ...
        float   xmax = 0.05
        int     xdivs = 49
        call K_C_int TABCREATE X {xdivs} {xmin} {xmax}
        int i
		...
		beta = 0.0
            ...
            setfield K_C_int X_A->table[{i}] {alpha}
            setfield K_C_int X_B->table[{i}] {alpha+beta}
            x = x + dx
        end
// Expand the tables to 3000 entries to use without interpolation
	setfield K_C_int X_A->calc_mode 0 X_B->calc_mode 0
	setfield K_C_int Xpower 1
	call K_C_int TABFILL X 3000 0
...
//  rather than being used in a differential equation.
        ...
        call K_C_int TABCREATE Z {xdivs} {xmin} {xmax}
        int i
        float x,dx,y
        dx = (xmax - xmin)/xdivs
        x = xmin
	    ...
	    /* activation will be computed as Z_A/Z_B */
            setfield K_C_int Z_A->table[{i}] {y}
            setfield K_C_int Z_B->table[{i}] 1.0
            x = x + dx
        end

	setfield K_C_int Z_A->calc_mode 0 Z_B->calc_mode 0
	setfield K_C_int Zpower 1
// Make it an instantaneous gate (no time constant)
	setfield K_C_int instant {INSTANTZ}
// Expand the table to 3000 entries to use without interpolation. 
	call K_C_int TABFILL Z 3000 0
...
// reader.
        addfield K_C_int addmsg1
        setfield K_C_int addmsg1  "../Ca_conc_int  . CONCEN Ca" 
end
